export const initTest = () => {
  ...
  const ROLE_IMAGES = {
    king: '/test/img/korol.webp',
    warrior: '/test/img/voin.webp',
    mage: '/test/img/mag.webp',
    poet: '/test/img/poet.webp',
    jester: '/test/img/shut.webp',
    advisor: '/test/img/sovetnik.webp'
  };
  ...
  const applyAnswer = () => {
    ...
    Object.entries(selectedAnswer.weights).forEach(([role, amount]) => {
      scores[role] += amount;
      totalScore += amount;
    });
  };
  ...
  const showResults = () => {
    ...
    if (imgSrc) {
      resultImageEl.setAttribute('src', imgSrc);
    } else {
